        ...
        override fun onLoadResource(view: WebView?, url: String?) {
            ...
            if (!isCalled) {
                if (url.toString().endsWith(suffix) || url.toString().endsWith(suffixTest)) {
                    ...
                    if (IS_FROM_NOTIFICATION && NFT_ID != 0 && SURVEY_ID != 0) {
                        val paramUrl = if (CUSTOM_URL.isNotEmpty()) {
                            "$CUSTOM_URL/mynft?nft_id=$NFT_ID&survey_id=$SURVEY_ID"
                        } else {
                            "$webUrl/mynft?nft_id=$NFT_ID&survey_id=$SURVEY_ID"
                        }
                        NFT_ID = 0
                        SURVEY_ID = 0
